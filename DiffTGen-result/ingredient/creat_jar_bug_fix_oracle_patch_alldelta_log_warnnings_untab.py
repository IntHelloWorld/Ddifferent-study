...
def create_oracle_and_delta(bugs):
    ...
    for patchfile in patch_list:
        ...
        if id not in bugs[proj].keys():
            ...
            continue
        create_oracle(bugs, patchfile, patch_name, proj, id, tool)
        create_delta(bugs, patchfile, patch_name, proj, id, tool)
    log("/************* Create oracle and delta finished! **************/")

def create_oracle(bugs, patchfile, patch_name, proj, id, tool):
    log("* %s-%s-%s oracle begin!"%(proj,id,tool))
    javaFileName = bugs[proj][id][0].split('/')[-1]
    buggy_path = work_path+'buggy/'+proj+'_'+id+'/'
    
    #创建工具名文件夹
    tool_path = work_path+proj+'/'+id+'/'+tool+'/'  
...
def create_delta(bugs, patchfile, patch_name, proj, id, tool):
